...
const CreateProfile = () => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      if (savedUser.profilePicture) {
        setPreviewProfilePic(savedUser.profilePicture);
      }
    }
    if (!token) navigate("/");
  }, [navigate, token]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      role === "jobSeeker" &&
      (!certFiles.length || selectedJobFields.length === 0)
    ) {
      setMessage(
        "Please upload certification photos and select at least one job field.",
      );
  ...
  return (
    <div className="page-container">
      ...
      <p className="intro-text">
        Welcome {user.firstName}! Let's complete your{" "}
        {role === "jobSeeker" ? "seeker" : "provider"} profile.
      </p>

      <div
        className="complaint-card-modern"
            ...
            >
              ...
              >
                {previewProfilePic ? (
                  ...
                  <span
                    style={{ fontSize: "40px", color: "var(--text-secondary)" }}
                  >
                    👤
                  </span>
                )}
                  ...
                  {jobFields.map((field) => (
                    <label
                      ...
                      style={{
                        ...
                        background: selectedJobFields.includes(field)
                          ? "var(--primary-green)"
                          : "var(--bg-secondary)",
                        color: selectedJobFields.includes(field)
                          ? "white"
                          : "var(--text-primary)",
                        ...
                        onChange={(e) => {
                          if (e.target.checked) {
                            ...
                            setSelectedJobFields(
                              selectedJobFields.filter((f) => f !== field),
                            );
                          }
                        }}
